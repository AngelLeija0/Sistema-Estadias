...
function compararNombres(a, b) {
  if (a.nivelAcademico !== b.nivelAcademico) {
  ...
  const apellidosA = `${a.apPaterno} ${a.apMaterno}`;
  const apellidosB = `${b.apPaterno} ${b.apMaterno}`;
...
router.post("/alumnos/eliminar", async (req, res) => {
  try {
    const idAlumno = req.body.idStudent;
    await Alumno.findByIdAndDelete(idAlumno);
...
router.post("/alumnos/proceso", async (req, res) => {
  try {
      ...
      if (enProceso(estadia)) {
        ...
        if (filtro.buscador) {
          ...
          if (numeroPartes.length >= 2) {
            const nombre = numeroPartes
              .slice(0, numeroPartes.length - 2)
              .join(" ");
            ...
            busqueda.$or.push({
              "datosPersonales.nombres.apPaterno": apPaterno,
            });
            busqueda.$or.push({
              "datosPersonales.nombres.apMaterno": apMaterno,
            });
          }
        }
    ...
    const alumnosFiltrados = alumnos.sort();
...
router.post("/alumnos/proceso/excel-general", async (req, res) => {
  try {
    ...
    const alumnosFiltrados = alumnos.sort(compararNombres);

    const data = alumnosFiltrados;
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=alumnos-en-proceso.xlsx"
    );
      ...
      if (procesoTerminado(estadia)) {
        ...
        if (alumno) {
          const infoAlumno = {
            ...
            grado: alumno.datosAcademicos.grado,
            grupo: alumno.datosAcademicos.grupo,
            proyecto: estadia.anteproyecto.datosProyecto.nombre,
          };
          alumnos.push(infoAlumno);
        }
      }
  ...
  try {
    ...
    if (filtro.buscador) {
      return res.status(500).json({
        message: "No se puede generar un archivo de Excel para un solo alumno.",
      });
    }
    if (filtro.año) {
      busqueda["cartaPresentacion.datosAcademicos.año"] = filtro.año;
...
router.patch("/alumno/perfil/cpa/modificar", async (req, res) => {
  try {
    ...
    estadia.cartaPresentacion.datosAlumno.telefonoCelular =
      datosAlumno.telefonoCelular;
    estadia.cartaPresentacion.datosAlumno.telefonoCasa =
      datosAlumno.telefonoCasa;
    ...
    estadia.cartaPresentacion.datosAcademicos.datosAcademicosAlumno.nivelAcademico =
      datosAcademicosAlumno.nivelAcademico;
    estadia.cartaPresentacion.datosAcademicos.datosAcademicosAlumno.turno =
      datosAcademicosAlumno.turno;
    estadia.cartaPresentacion.datosAcademicos.datosAcademicosAlumno.carrera =
      datosAcademicos.carrera;
    estadia.cartaPresentacion.datosAcademicos.datosAcademicosAlumno.area =
      datosAcademicos.area;
    estadia.cartaPresentacion.datosAcademicos.datosAcademicosAlumno.grado =
      datosAcademicosAlumno.grado;
    estadia.cartaPresentacion.datosAcademicos.datosAcademicosAlumno.grupo =
      datosAcademicosAlumno.grupo;
    ...
    estadia.cartaPresentacion.datosEmpresa.nombreEmpresa =
      datosEmpresa.nombreEmpresa;
    estadia.cartaPresentacion.datosEmpresa.nombreEmpresario =
      datosEmpresa.nombreEmpresario;
    estadia.cartaPresentacion.datosEmpresa.puestoEmpresario =
      datosEmpresa.puestoEmpresario;
...
router.post("/alumno/perfil/cpa/imprimir", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=carta_presentacion.docx"
    );
...
router.patch("/alumno/perfil/cpa/eliminar", async (req, res) => {
  try {
    ...
    if (estadia.cartaPresentacion) {
      estadia.cartaPresentacion = null;
      await Estadia.findByIdAndUpdate(estadia._id, estadia);
      res.json("Eliminado correctamente");
    } else {
...
router.patch("/alumno/perfil/anteproyecto/modificar", async (req, res) => {
  try {
    ...
    estadia.anteproyecto.datosEmpresa.emailEmpresario =
      datosEmpresa.emailEmpresario ||
      estadia.anteproyecto.datosEmpresa.emailEmpresario;
    estadia.anteproyecto.datosEmpresa.telefonoEmpresario =
      datosEmpresa.telefonoEmpresario ||
      estadia.anteproyecto.datosEmpresa.telefonoEmpresario;

    const datosProyecto = anteproyecto.datosProyecto;
    estadia.anteproyecto.datosProyecto.nombre =
      datosProyecto.nombre || estadia.anteproyecto.datosProyecto.nombre;
    estadia.anteproyecto.datosProyecto.objetivo =
      datosProyecto.objetivo || estadia.anteproyecto.datosProyecto.objetivo;
    estadia.anteproyecto.datosProyecto.descripcion =
      datosProyecto.descripcion ||
      estadia.anteproyecto.datosProyecto.descripcion;

    estadia.anteproyecto.estado.nombre =
      anteproyecto.estado.nombre || estadia.anteproyecto.estado.nombre;
    estadia.anteproyecto.estado.motivo =
      anteproyecto.estado.motivo || estadia.anteproyecto.estado.motivo;
    estadia.anteproyecto.estado.fecha =
      anteproyecto.estado.fecha || estadia.anteproyecto.estado.fecha;

    estadia.anteproyecto.fechaRegistro =
      anteproyecto.fechaRegistro || estadia.anteproyecto.fechaRegistro;
...
router.post("/alumno/perfil/documentos/documento-toggle", async (req, res) => {
  try {
    ...
    const estadia = await Estadia.findOne(filtro);
    estadia.documentos[documento].estado = estado;
    estadia.documentos[documento].motivo = motivo;
    estadia.documentos[documento].fecha = new Date();

    await Estadia.findByIdAndUpdate(estadia._id, estadia);

    res.json({ documento, estado, motivo, fecha: new Date() });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
});
...
router.post("/asesores", async (req, res) => {
  try {
    ...
    const asesoresFiltrados = arrAsesores.sort(compararNombres);
    res.json(asesoresFiltrados);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
async function saveAsesor(asesor, res, body) {
  try {
    ...
    asesor.datosPersonales.nombres.nombre =
      body.asesor.datosPersonales.nombres.nombre ||
      asesor.datosPersonales.nombres.nombre;
    asesor.datosPersonales.nombres.apPaterno =
      body.asesor.datosPersonales.nombres.apPaterno ||
      asesor.datosPersonales.nombres.apPaterno;
    asesor.datosPersonales.nombres.apMaterno =
      body.asesor.datosPersonales.nombres.apMaterno ||
      asesor.datosPersonales.nombres.apMaterno;

    asesor.datosPersonales.privado.email =
      body.asesor.datosPersonales.privado.email ||
      asesor.datosPersonales.privado.email;
    asesor.datosPersonales.privado.telefono =
      body.asesor.datosPersonales.privado.telefono ||
      asesor.datosPersonales.privado.telefono;
    asesor.datosPersonales.privado.username =
      body.asesor.datosPersonales.privado.username ||
      asesor.datosPersonales.privado.username;

    asesor.datosAcademicos.carrera =
      body.asesor.datosAcademicos.carrera || asesor.datosAcademicos.carrera;
...
router.post("/asesor/perfil/alumnos", async (req, res) => {
  try {
    ...
    for (const estadia of estadias) {
      try {
        if (
          estadia.documentos?.etapas[5]?.estado !== "Aceptada" &&
          estadia.avance?.revision?.estado !== "true"
        ) {
          const idAlumno = estadia._doc.idAlumno;
          const busqueda = {
            _id: new ObjectId(idAlumno),
...
router.post("/administrador/crear", async (req, res) => {
  try {
    const password = req.body.administrador.password;
    ...
    bcrypt.hash(password, 10, async (err, hash) => {
      ...
      const administrador = new Administrador({
        username: req.body.administrador.username,
        email: req.body.administrador.email,
        password: hash,
        carrera: req.body.administrador.carrera,
...
router.patch("/administrador/cambiar-password", async (req, res) => {
  try {
    const idAdministrador = req.body.idAdmin;
    ...
    bcrypt.hash(password, 10, async (err, hash) => {
      ...
      await Administrador.findByIdAndUpdate(administrador._id, administrador);

      res.status(201).json("Modificado correctamente");
    });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
...
router.post("/administrador/eliminar", async (req, res) => {
  try {
    const idAdministrador = req.body.idAdmin;
    const administrador = await Administrador.findByIdAndDelete(
      idAdministrador
    );
    if (administrador) {
    ...
    res
      .status(204)
      .json({ message: `Administrador ${idAdministrador} no encontrado` });
  } catch (error) {
    res.status(204).json({ message: error.message });
  }
});
