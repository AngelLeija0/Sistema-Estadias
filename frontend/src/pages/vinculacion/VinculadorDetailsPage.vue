            ...
            <q-card style="width: 400px">
              <q-card-section class="q-pa-lg text-center">
                ...
                <p style="font-size: 18px; padding-top: 20px">
                  ¿Estas seguro de querer eliminar el vinculador
                  {{ admin.datosPersonales.nombres.nombre }}
                  {{ admin.datosPersonales.nombres.apPaterno }}?
                </p>
              </q-card-section>
...
export default defineComponent({
  ...
  setup() {
    ...
    function loadAdmin(updatedAdmin = null) {
      try {
        if (updatedAdmin !== null) {
          ...
          api
            .post(
              `http://localhost:3000/vinculacion/vinculador/perfil`,
              {
                vinculador: idAdmin.value,
              }
            )
            .then((res) => {
    ...
    function deleteAdmin() {
      api
        .post(
          `http://localhost:3000/vinculacion/vinculador/perfil/borrar`,
          {
            vinculador: idAdmin._value,
          }
        )
        .then((res) => {
          console.log(res.data);
          if (res.data === "Vinculador eliminado correctamente") {
            router.push({ name: "vinculacion-listaVinculadores" });
          }
        })
        .catch((err) => {
          console.error(err);
    ...
    function saveEditInfo() {
      ...
      api
        .patch(
          `http://localhost:3000/vinculacion/vinculador/perfil/modificar`,
          {
            vinculador: updatedAdmin,
          }
        )
        .then((res) => {
          if (res.data.msg) {
